...
const MiddlePage = ({
  ...
  const [moodSelectionMeta, setMoodSelectionMeta] = useState(null);
  const [isCollapsed, setIsCollapsed] = useState(true);
  const [draggedIndex, setDraggedIndex] = useState(null);
  const [isPlayerModalOpen, setIsPlayerModalOpen] = useState(false);
  const [selectedAlbumId, setSelectedAlbumId] = useState(null);
  const [selectedArtistForProfile, setSelectedArtistForProfile] =
    ...
    const fetchUserData = async () => {
      ...
      try {
        const [likedRes, playlistRes] = await Promise.all([
          axios.get(
            `${import.meta.env.VITE_API_BASE_URL}/api/user/liked-songs`,
          ),
          axios.get(
            `${import.meta.env.VITE_API_BASE_URL}/api/playlists/my-playlists`,
          ),
        ]);
  ...
  const fetchMoodSongs = async (selection) => {
    const mood = typeof selection === "string" ? selection : selection?.mood;
    const tags = Array.isArray(selection?.tags) ? selection.tags : [];
    if (!mood) return;
    setMoodSelectionMeta({
      mood,
      tag: selection?.selectedTag || tags[0] || null,
    });
  ...
  const createPlaylist = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/playlists/create`,
  ...
  const handleProgressBarClick = (e) => {
    ...
    // Use duration prop if available, else fallback to audioRef
  ...
  const addToLiked = async (song) => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/user/like-song`,
        { songId: song.id },
      );
      if (res.data.success) {
        if (res.data.isLiked) {
  ...
  const addToPlaylist = async (playlistId, songId) => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/playlists/add-song`,
        { playlistId, songId },
      );
      if (res.data.success) {
      ...
      showNotification(
        error.response?.data?.message || "Error adding to playlist",
      );
    }
  };
  ...
  const removeFromPlaylist = async (songId) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/playlists/remove-song`,
        { playlistId: selectedPlaylist._id, songId },
      );
      if (res.data.success) {
        const newSongs = selectedPlaylist.songs.filter(
          (s) => (s.id || s._id) !== songId,
        );
  ...
  const playFromQueue = (queueIndex) => {
    setQueue((prev) => {
      const tail = prev.slice(queueIndex);
      if (tail.length === 0) return prev;

      const normalizedTail = tail.map((song) => ({
        ...song,
        id: song.id || song._id,
        cover: song.cover || song.coverUrl,
        audio: song.audio || song.audioUrl,
      ...
      const [nextNowPlaying, ...remainingQueue] = normalizedTail;
      setPlaybackSongs(normalizedTail);
      setCurrentSong(nextNowPlaying);
      setIsPlaying(true);

      // Remove everything above clicked song and keep only upcoming tail.
      return remainingQueue;
    });
  };

  const playFromPlaybackTail = (song) => {
    ...
    const startIndex = playbackSongs.findIndex(
      (s) =>
        (s.id || s._id)?.toString() === (normalizedSong.id || normalizedSong._id)?.toString(),
    );
    if (startIndex < 0) return;

    const tail = playbackSongs.slice(startIndex).map((s) => ({
      ...s,
      id: s.id || s._id,
    ...
    setPlaybackSongs(tail);
    setCurrentSong(tail[0]);
    setIsPlaying(true);
  };
  ...
  return (
    <div
      ...
      style={{
        height:
          active !== "Home" ? "calc(100vh - 80px)" : "calc(100vh - 165px)",
      }}
    >
      {/* Sidebar */}
      {/* OFFLINE SPLASH OVERLAY */}
      {!isOnline &&
        (active === "Home" || active === "Discover" || active === "Trends") && (
          <div
            style={{
              flex: 1,
              display: "flex",
          ...
          >
            ...
            >
              You're currently offline. But don't worry, your music doesn't have
              to stop!
            </p>
            <button
      ...
      <div className={styles.right}>
        <div
          className={`${styles.contentWrapper} ${currentSong ? styles.withMiniPlayer : ""}`}
        >
          <div className={styles.mainContent}>
                ...
                >
                  ...
                  {currentSong && (
                    <div className={styles.queueItemActive}>
                      ...
                      <div>
                        ...
                        <p className={styles.queueArtist}>
                          {currentSong.artist}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className={styles.queueSection}>
                    <div className={styles.queueSectionHeader}>
                      <h3>Next in Queue</h3>
                      {queue.length > 0 && (
                        <span onClick={() => setQueue([])}>Clear</span>
                      )}
                    </div>
                    {queue.length === 0 ? (
                      ...
                      {playbackSongs
                        ...
                        .map((song, idx) => (
                          <div
                            ...
                            onClick={() => playFromPlaybackTail(song)}
                          >
                            <img src={song.cover} alt={song.title} />
                            <div className={styles.queueInfo}>
                              <p className={styles.queueTitle}>{song.title}</p>
            ...
            {active === "Upload" && (
              <UploadPage songs={allSongs} setSongs={setAllSongs} />
            )}
            ...
            {active === "PlaylistView" && selectedPlaylist && (
              <SongListSection
                ...
                handleSongClick={(song) => {
                  const formatted = selectedPlaylist.songs.map((s) => ({
                    ...s,
                    id: s._id,
                    cover: s.coverUrl,
                    audio: s.audioUrl,
                    ...
                    allSongs.filter(
                      (s) =>
                        s.artist
                          .toLowerCase()
                          .includes(
                            selectedArtistForProfile.name.toLowerCase(),
                          ) ||
                        (s.artists &&
                          s.artists.includes(selectedArtistForProfile.name)),
                    ),
                  );
                  handleSongClick(song);
                }}
            ...
            {active === "MoodAnalyser" && (
              <MoodAnalyser onMoodSelect={fetchMoodSongs} />
            )}

            {active === "MoodAnalyser" && moodSongs.length > 0 && (
              <div style={{ marginTop: "10px" }}>
                <SongListSection
                  title={
                    moodSelectionMeta?.tag
                      ? `${moodSelectionMeta.tag} Songs`
                      : moodSelectionMeta?.mood
                        ? `${moodSelectionMeta.mood[0].toUpperCase()}${moodSelectionMeta.mood.slice(1)} Songs`
                      : "Mood Songs"
                  }
                  songs={moodSongs}
                  currentSong={currentSong}
                  handleSongClick={(song) => {
                    setPlaybackSongs(moodSongs);
                    setCurrentSong(song);
                    setIsPlaying(true);
                  }}
                  draggedIndex={draggedIndex}
          ...
          {currentSong &&
            (active === "Discover" ||
              ...
              <div className={styles.sidePlayer}>
                <div
                  className={styles.sidePlayerCard}
                ...
                >
                  ...
                  <div className={styles.sideCardBody}>
                    ...
                    <div
                      className={styles.sideProgressContainer}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div
                        className={styles.sideProgressBar}
          ...
          >
            ...
            <div className={styles.glassBody}>
              ...
              <div className={styles.glassProgressRow}>
                <span className={styles.glassTime}>
                  {formatTime(currentTime)}
                </span>
                <div
                  className={styles.glassProgressBar}
                  onClick={handleProgressBarClick}
                >
                  <div
...
const SongListSection = ({
  ...
  const feedbackToastTimers = useRef({});

  useEffect(() => {
    const timers = feedbackTimers.current;
    const toastTimers = feedbackToastTimers.current;
    return () => {
  ...
  const triggerFeedbackEffect = (songId, liked) => {
    const key = `${songId}-${liked ? "like" : "skip"}`;
    if (feedbackTimers.current[key]) {
      clearTimeout(feedbackTimers.current[key]);
    }

    setFeedbackPulse((prev) => ({ ...prev, [key]: true }));
    feedbackTimers.current[key] = setTimeout(() => {
      setFeedbackPulse((prev) => ({ ...prev, [key]: false }));
      delete feedbackTimers.current[key];
    }, 380);
  };

  const triggerFeedbackToast = (songId, liked) => {
    const label = liked ? "Great pick. Feedback saved." : "Noted. We will refine recommendations.";
    if (feedbackToastTimers.current[songId]) {
      clearTimeout(feedbackToastTimers.current[songId]);
    }

    setFeedbackToast((prev) => ({
      ...prev,
      [songId]: { visible: true, label },
    }));

    feedbackToastTimers.current[songId] = setTimeout(() => {
      setFeedbackToast((prev) => ({
        ...prev,
        [songId]: { ...prev[songId], visible: false },
      }));
      delete feedbackToastTimers.current[songId];
    }, 1400);
  };
              ...
              >
                ...
                {sendFeedback && (
                  <div className={styles.feedbackActions}>
                    <button
                      className={`${styles.feedbackBtn} ${styles.feedbackLike} ${
                        feedbackPulse[`${song.id || song._id}-like`]
                      ...
                      onClick={(e) => {
                        ...
                        triggerFeedbackEffect(songId, true);
                        triggerFeedbackToast(songId, true);
                        sendFeedback(songId, true);
                      }}
                    >
                    ...
                    <button
                      className={`${styles.feedbackBtn} ${styles.feedbackSkip} ${
                        feedbackPulse[`${song.id || song._id}-skip`]
                          ? styles.feedbackPulse
                          : ""
                      }`}
                      aria-label="Skip recommendation"
                      title="Skip recommendation"
                      onClick={(e) => {
                        e.stopPropagation();
                        const songId = song.id || song._id;
                        triggerFeedbackEffect(songId, false);
                        triggerFeedbackToast(songId, false);
                        sendFeedback(songId, false);
                      }}
                    >
                      <SkipForward size={16} strokeWidth={2.2} />
                    </button>
                    {feedbackToast[song.id || song._id]?.visible && (
                      <span className={styles.feedbackToast}>
                        {feedbackToast[song.id || song._id].label}
                      </span>
                    )}
                  </div>
                )}
